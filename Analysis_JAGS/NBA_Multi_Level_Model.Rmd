...
For JAGS, I clean my data in a way that will work better. My github repo has more information about this project, as well as how I cleaned the data: <https://github.com/yjchen9596/Multilevel_Model_NBAplayers_Salary>

For the model, I use salary as y predictors and total points of the season by players as x. The data is a combination of NBA players salary history with 2016-2017 season and 2017-2018 season performance. To compare with previous project results, I kept the same two seasons for the dataset as well.
...
```{r}
basic.linear.model <- lmer(salary ~  pts + (1 | position) + (1 | age_group), data = NBA)
...
```{r}
knitr::kable(bind_rows(nba.mcmc1.convergence[1:4][1:4]))
```


Copied one of the diagnostic from the txt folder
...
```{r}
knitr::kable(bind_rows(nba.mcmc3.convergence[1:4][1:4]))
```
Copied one of the diagnostic from the txt folder
...
You need a sample size of at least 5141 with these values of q, r and s
